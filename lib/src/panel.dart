...
class _SlidingUpPanelState extends State<SlidingUpPanel>
  ...
  void initState() {
    ...
    _ac = AnimationController(
      ...
      value: widget.defaultPanelState == PanelState.CLOSED
        ...
        if (widget.onPanelSlide != null && _ac != null) {
  ...
  void _onGestureEnd(Velocity v) {
    ...
    if (_ac?.isAnimating ?? true) return;
    ...
    if (v.pixelsPerSecond.dy.abs() >= minFlingVelocity) {
      ...
      if (widget.panelSnapping && widget.snapPoint != null) {
        if (v.pixelsPerSecond.dy.abs() >= kSnap * minFlingVelocity ||
            minDistance == d2Snap) {
          _ac!.fling(velocity: visualVelocity).then((value) => setState(() {}));
        } else {
  ...
  void _flingPanelToPosition(double targetPos, double velocity) {
    ...
    _ac!.animateWith(simulation).then((value) => setState(() {}));
  }
  ...
  //close the panel
  Future<void> _close() {
    return _ac?.fling(velocity: -1.0).then((value) => setState(() {})) ??
        Future.value();
  }

  //open the panel
  Future<void> _open() {
    return _ac?.fling().then((value) => setState(() {})) ?? Future.value();
  }
  ...
  double get _panelPosition => _ac?.value ?? 0.0;

  //returns whether or not
  //the panel is still animating
  bool get _isPanelAnimating => _ac?.isAnimating ?? false;
...
class PanelController {
  ...
  Future<void> close() {
    ...
    return _panelState?._close() ?? Future.value();
  }
  ...
  Future<void> open() {
    assert(isAttached, "PanelController must be attached to a SlidingUpPanel");
    return _panelState?._open() ?? Future.value();
  }

  /// Hides the sliding panel (i.e. is invisible)
  Future<void> hide() {
    assert(isAttached, "PanelController must be attached to a SlidingUpPanel");
    return _panelState?._hide() ?? Future.value();
  }
  ...
  Future<void> show() {
    assert(isAttached, "PanelController must be attached to a SlidingUpPanel");
    return _panelState?._show() ?? Future.value();
  }
